...
public class OutPut {
	...
	public synchronized void WriteFile(ServiceMessage se) {
		/*
		 @REQUIRES:s!=null 
		 @
		 @
		 @MODIFIES:Servicemessage.txt
		 @
		 @EFFECTS:将s输入到ServiceMessage.txt中
		 @THREAD_REQUIRES:none
		 @
		 @THREAD_EFFECTS:\locked()
		 @
		 */
		File file = new File("ServiceMessage.txt");
		for (int i = 0; i < se.request.size(); i++) {
			String content = se.request.get(i) + "\r\n";
